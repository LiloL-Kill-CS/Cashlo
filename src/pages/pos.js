...
export default function POSPage() {
    ...
    const { transactions, createTransaction } = useTransactions((user?.owner_id || user?.id), user?.role, user?.id);
    const { heldOrders, holdOrder, recallOrder, deleteHeldOrder } = useHeldOrders((user?.owner_id || user?.id));
